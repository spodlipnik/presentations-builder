...
- Read `docs/talk.yaml` for topic and intent context
- Read `docs/vision.md` for the speaker's personal angle
- Read `${user_config.assets_path}/config.yaml` and extract the `design_tokens:` section for colors and fonts (config.yaml is the single source of truth for visual style as of v2.0)
...
- Background: transparent or design_tokens.color.background
...
**Style rules:**
- Same color palette as CHART (from config.yaml > design_tokens)
...
**How:** Generate a detailed prompt file `[GENERATE]-description.txt` with style context from config.yaml > design_tokens. The user generates the image externally and places it in `images/`.
...
- **Consistent:** Same color palette (config.yaml > design_tokens), same style across all created visuals
